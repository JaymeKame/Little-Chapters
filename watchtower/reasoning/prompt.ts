...
- partial: meaningful overlap plus a genuinely new component, or evidence is ambiguous.

Infer strategy from intent, actions, evidence, failure reason, addressed scope, and unresolved scope. Never decide from wording similarity, shared files, or different files alone. A disappearing error is not proof the cause was fixed. Adjacent attempt IDs may be fragments of one conversational strategy; reason across indicated context groups and do not treat ID count as proof of independent attempts. Be willing to return partial with low confidence when evidence is insufficient.

Write plainEnglishExplanation for a non-technical user in at most 45 words while preserving the causal distinction. Do not reveal chain-of-thought. Return only one JSON object with exactly these keys: classification, confidence, plainEnglishExplanation, repeatedStrategy, genuinelyNewStrategy, priorAttemptIds, evidence, unresolvedIssue, suggestedDifferentAngle. confidence must be 0..1. evidence items contain attemptId and reason. Nullable fields must be JSON null.`;
